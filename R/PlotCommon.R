...
################################################################################################################################
################################################################################################################################
PlotBarFromList<-function(d, ylab='', color='#88888888', plotly=FALSE, individual=FALSE) {
  ...
  if (!plotly) {
    ...
    if (individual) {
      lims[1] <- min(0, min(sapply(d, function(d) min(d, na.rm=TRUE)), na.rm=TRUE));
      lims[2] <- max(0, max(sapply(d, function(d) max(d, na.rm=TRUE)), na.rm=TRUE));
      rng <- lims[2]-lims[1];
    }
